...
//
	...
	public extension ChildControllerEmbeddable where Self: UIViewController {
		func embedd(controller: UIViewController?) {
		...
		func unembedd(controller: UIViewController?) {
			guard let controller = controller else {
				return
			}
