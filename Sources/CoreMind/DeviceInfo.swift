...
public struct DeviceInfo {
	...
	public init(bundle: Bundle = .main) {
		self.bundle = BundleInfo(bundle: bundle)
			...
			model = DeviceInfo.deviceName() ?? device.model
			...
			model = DeviceInfo.sysctlWithName("hw.machine")
			operatingSystem = info.operatingSystemVersionString
			name = Host.current().localizedName ?? "unknown host"
		#endif
	}

	// eg. com.company.MyApp/1 rv:123 (iPhone5,2 iOS/12.0 CFNetwork/808.3 Darwin/16.3.0)
	public func userAgentString() -> String {
		"\(appNameAndVersion(bundleInfo: bundle)) (\(model) \(operatingSystem) \(DeviceInfo.CFNetworkVersion()) \(DeviceInfo.darwinVersion()))"
	}
